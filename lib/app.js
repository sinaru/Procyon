...
class App {
  ...
  async processRequest() {
    const resource = this.route.resource();
    const module = await import(`${procyon.subPath()}/app/controllers/${resource.className()}.js`);
  ...
  setBasePath() {
    ...
    this.cachedBasePath = `${url.protocol}//${url.host}/${url.pathname.split('/')[1]}`;
  }
...
window.procyon = window.procyon ? window.procyon : new App();
